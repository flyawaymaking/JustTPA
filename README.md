...
- **`/justtpa reload`** - перезагрузить конфиг
...
- 🔄 **Поддержка кастомного перевода** - перевод можно настроить в `config.yml`

## Установка
1. Скачайте **последний релиз** из раздела [Releases](../../releases)
2. Поместите его в папку /plugins
3. Перезапустите сервер
4. Настройте `config.yml` и выполните команду `/justtpa reload`
...
- `justtpa.user` - разрешения на выполнение всех команд (tpa/tpahere/tpaccept/tpdeny/tpcancel)
- `justtpa.reload` - перезагрузка конфига
- `justtpa.*` - полные права плагина
